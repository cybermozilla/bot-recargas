...
TOKEN_BOT = os.environ.get("7864266536:AAHW3BMrhRqDzeUsaydQQ6Pum5KbCpG8GEM", "").strip()
API_KEY_TIENDA = os.environ.get("2fc6bc5920314acce1467adb2e95dbd369e7312f31a7c465f6aef0fb86d7537d", "").strip()
...
def enviar_mensaje(chat_id, texto):
    ...
    print(f"TOKEN: '{TOKEN_BOT[:10]}...'")
    resp = requests.post(url, json={
        "chat_id": chat_id,
        "text": texto
    })
    print(f"Respuesta Telegram: {resp.status_code} {resp.text}")
...
def webhook():
    ...
    if texto == "/start":
        enviar_mensaje(chat_id, "✅ Bot conectado. Usa /saldo")
    elif texto == "/saldo":
        consultar_saldo(chat_id)
    else:
        enviar_mensaje(chat_id, "Usa /saldo")
    return jsonify({"status": "ok"})
